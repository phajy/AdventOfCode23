...
for start_row in range(1, size(mirror_map, 1))
    beam_map = fill('.', size(mirror_map))
    trace_beam!(beam_map, mirror_map, start_row, 1, 0, 1)
    best_n_energized = maximum([best_n_energized, count(x -> x != '.', beam_map)])
    beam_map = fill('.', size(mirror_map))
    trace_beam!(beam_map, mirror_map, start_row, size(mirror_map, 2), 0, -1)
    best_n_energized = maximum([best_n_energized, count(x -> x != '.', beam_map)])
end

for start_column in range(1, size(mirror_map, 2))
    beam_map = fill('.', size(mirror_map))
    trace_beam!(beam_map, mirror_map, 1, start_column, 1, 0)
    best_n_energized = maximum([best_n_energized, count(x -> x != '.', beam_map)])
    beam_map = fill('.', size(mirror_map))
    trace_beam!(beam_map, mirror_map, size(mirror_map, 2), start_column, -1, 0)
    best_n_energized = maximum([best_n_energized, count(x -> x != '.', beam_map)])
end

println("Maximum number of energized squares = ", best_n_energized)
