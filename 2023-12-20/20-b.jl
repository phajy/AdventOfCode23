...
function process_pulse(pulse, n_presses)
    global cycle_lengths
    outcome = []
    ...
    if pulse_to[1] == 'c'
        ...
        if all_high
            pulse_to_send = -1
            if pulse[2] ∈ ["bp", "bd", "xn", "pm"]
                @debug pulse[2], " is hit after ", n_presses
                push!(cycle_lengths, n_presses)
            end
        else
            pulse_to_send = 1
        end
...
n_low = 0
cycle_lengths = []
for cycles in range(1, 100_000_000)
    # println("Button presses = ", cycles)
    ...
    while length(pulses) > 0
        ...
        new_pulses = process_pulse(pulse_to_process, cycles)
...
println("The answer is ", lcm(cycle_lengths[1], cycle_lengths[2], cycle_lengths[3], cycle_lengths[4]))

# 3767 x 3769 x 3881 x 4019 = 221453937522197
