...
function subtour_elimination_callback(cb_data)
    ...
    # get all steps on the route including sub-tours
    from = []
    to = []
    to_eliminate = []
    for k in keys(x)
        if callback_value(cb_data, x[k]) > 0.5
            push!(from, (k[1], k[2]))
            push!(to, (k[3], k[4]))
            push!(to_eliminate, k)
        end
    ...
    # remove valid route
    cur_pos = (1, start_column)
    done = false
    while cur_pos ≠ (n_rows, end_column)
        index = findfirst(isequal(cur_pos), from)
        cur_pos = to[index]
        deleteat!(from, index)
        deleteat!(to, index)
        deleteat!(to_eliminate, index)
    end
